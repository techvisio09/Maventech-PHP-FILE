...
- **[Feb 2026]** Admin-side Live Chat in Lead Management — two-way real-time conversation between admin and website visitors:
  ...
  - **Verified end-to-end** via screenshot tool: customer message inserted → admin sidebar badge appears (`3`), online customer's row gets green pulsing dot, online chat pill turns green, offline chat pills stay rich gray, red notification dot pulses on unread pills. Admin opens drawer → "Online" green pill, customer bubbles on right (light blue), admin reply renders on left (light green). After open, `read_at` is set on all customer messages and the badge clears on next poll.
