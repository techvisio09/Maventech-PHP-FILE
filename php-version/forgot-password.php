        ...
        try {
            ...
            if ($user) {
                ...
                $body = ''
                    ...
                    . '</div>';
                send_email((string)$user['email'], 'Reset your ' . SITE_BRAND . ' password', $body);
            }
        } catch (Throwable $e) {
...
<div class="container py-5" style="min-height:60vh;">
  <div class="mx-auto" style="max-width:440px;">
    <div class="card" style="border:1px solid var(--border,#e5e7eb);border-radius:14px;padding:28px 26px;box-shadow:0 10px 30px rgba(15,23,42,.06);">
      ...
      <p class="text-secondary small mb-4">No problem — enter your registered email and we'll send you a reset link.  The link expires in 60 minutes.</p>

      <?php if ($sent): ?>
        <div class="alert alert-success small" data-testid="forgot-success" style="border-radius:10px;line-height:1.5;">
          <i class="bi bi-check2-circle me-1"></i>If an account exists for that email, a password-reset email is on its way.  Check your inbox (and spam folder) for a message from <strong><?= esc(SITE_BRAND) ?></strong>.
        </div>
      ...
      <?php else: ?>
        ...
        <form method="post" data-testid="forgot-form">
          <label class="form-label small fw-semibold mb-1">Registered email</label>
          <input type="email" name="email" class="form-control mb-3" required autofocus placeholder="you@example.com" data-testid="forgot-email" value="<?= esc((string)($_POST['email'] ?? '')) ?>">
