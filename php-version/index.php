            ...
            <div class="col-sm-5">
              <a href="product.php?slug=<?= esc($feat['slug']) ?>" class="d-block">
                <div class="spotlight-img-wrap rounded-4 p-3 position-relative">
                  ...
                  <img src="<?= esc($feat['image']) ?>" alt="<?= esc(product_img_alt($feat)) ?>" title="<?= esc($feat['name']) ?>">
                </div>
              </a>
            </div>
            <div class="col-sm-7">
              <div class="small mb-1"><?= render_stars((float)$feat['rating']) ?> <span class="text-secondary">(<?= (int)$feat['reviews'] ?> reviews)</span></div>
              <h3 class="h4 fw-bold mb-2"><a href="product.php?slug=<?= esc($feat['slug']) ?>" class="text-decoration-none text-body"><?= esc($feat['name']) ?></a></h3>
              <p class="small text-secondary mb-3">Genuine lifetime license key with instant email delivery, free installation support and a 30-day money-back guarantee.</p>
          ...
          <?php foreach ($sideSellers as $p): ?>
            <div class="card side-product-row p-3 d-flex flex-row align-items-center gap-3" data-testid="side-seller-<?= esc($p['slug']) ?>">
              ...
              <div class="flex-grow-1 min-w-0">
                ...
                <div class="small"><?= render_stars((float)$p['rating']) ?> <span class="text-secondary">(<?= (int)$p['reviews'] ?>)</span></div>
